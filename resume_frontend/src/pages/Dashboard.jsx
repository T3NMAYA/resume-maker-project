...
import { deleteUser } from "../api/ResumeService";
...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        if (loggedInUser) {
            ...
            const savedResumes = JSON.parse(localStorage.getItem(`resumes_${loggedInUser.email}`)) || [];
    ...
    const handleDeleteAccount = async () => {
        if (window.confirm("Are you sure you want to permanently delete your account and all your data?")) {
            try {
                ...
                localStorage.removeItem(`resumes_${user.email}`); // Clear saved resumes too
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                                {resumes.map(resume => (
                                    <div key={resume.id} className="card bg-base-100/50 shadow-lg">
                                        <div className="card-body">
                                            <h3 className="card-title">Resume - {new Date(resume.createdAt).toLocaleDateString()}</h3>
